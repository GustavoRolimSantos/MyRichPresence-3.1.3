...
public abstract class WebSocketServer extends AbstractWebSocket implements Runnable
{
    ...
    private boolean doRead(final SelectionKey key, final Iterator<SelectionKey> i) throws InterruptedException, IOException {
    ...
    private void doServerShutdown() {
        ...
        if (this.selector != null) {
            try {
                ...
                WebSocketServer.log.error("IOException during selector.close", e);
                this.onError(null, e);
            }
        }
        if (this.server != null) {
            try {
                ...
                WebSocketServer.log.error("IOException during server.close", e);
                this.onError(null, e);
            }
        }
    }
    ...
    private void handleIOException(final SelectionKey key, final WebSocket conn, final IOException ex) {
        if (conn != null) {
            ...
            if (channel != null && channel.isOpen()) {
                ...
                WebSocketServer.log.trace("Connection closed because of exception", ex);
            }
        }
    }
    
    private void handleFatal(final WebSocket conn, final Exception e) {
        WebSocketServer.log.error("Shutdown due to fatal error", e);
        this.onError(conn, e);
        if (this.decoders != null) {
            for (final WebSocketWorker w : this.decoders) {
                w.interrupt();
        ...
        try {
            ...
            WebSocketServer.log.error("Error during shutdown", e2);
            this.onError(null, e2);
        }
        catch (InterruptedException e3) {
            Thread.currentThread().interrupt();
            WebSocketServer.log.error("Interrupt during stop", e);
    ...
    protected boolean removeConnection(final WebSocket ws) {
        ...
        synchronized (this.connections) {
            if (this.connections.contains(ws)) {
                ...
                WebSocketServer.log.trace("Removing connection which is not in the connections collection! Possible no handshake recieved! {}", ws);
            }
        }
    ...
    private void doBroadcast(final Object data, final Collection<WebSocket> clients) {
        ...
        for (final WebSocket client : clients) {
            if (client != null) {
    ...
    static {
        log = LoggerFactory.getLogger(WebSocketServer.class);
    ...
    public class WebSocketWorker extends Thread
    {
        ...
        public WebSocketWorker() {
            ...
            this.setUncaughtExceptionHandler(new UncaughtExceptionHandler() {
                ...
                public void uncaughtException(final Thread t, final Throwable e) {
                    WebSocketServer.log.error("Uncaught exception in thread {}: {}", t.getName(), e);
                }
            });
        }
        ...
        private void doDecode(final WebSocketImpl ws, final ByteBuffer buf) throws InterruptedException {
            try {
                ...
                WebSocketServer.log.error("Error while reading from remote connection", e);
            }
            finally {
