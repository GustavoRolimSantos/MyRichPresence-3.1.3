...
public class SSLSocketChannel2 implements ByteChannel, WrappedByteChannel
{
    ...
    protected void createBuffers(final SSLSession session) {
        ...
        if (this.inData.remaining() != 0 && SSLSocketChannel2.log.isTraceEnabled()) {
            SSLSocketChannel2.log.trace(new String(this.inData.array(), this.inData.position(), this.inData.remaining()));
        }
        this.inData.rewind();
        this.inData.flip();
        if (this.inCrypt.remaining() != 0 && SSLSocketChannel2.log.isTraceEnabled()) {
            SSLSocketChannel2.log.trace(new String(this.inCrypt.array(), this.inCrypt.position(), this.inCrypt.remaining()));
        }
        this.inCrypt.rewind();
        this.inCrypt.flip();
