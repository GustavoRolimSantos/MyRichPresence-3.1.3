...
public class SSLSocketChannel implements WrappedByteChannel, ByteChannel
{
    ...
    public SSLSocketChannel(final SocketChannel inputSocketChannel, final SSLEngine inputEngine, final ExecutorService inputExecutor, final SelectionKey key) throws IOException {
        ...
        if (this.doHandshake()) {
            ...
            try {
                ...
                SSLSocketChannel.log.error("Exception during the closing of the channel", e);
            }
        }
    }
        ...
        if (bytesRead > 0 || this.peerNetData.hasRemaining()) {
            ...
            if (this.peerNetData.hasRemaining()) {
                ...
                try {
                    ...
                    SSLSocketChannel.log.error("SSLExcpetion during unwrap", e);
                    throw e;
                }
    ...
    private void handleEndOfStream() throws IOException {
        try {
            ...
            SSLSocketChannel.log.error("This engine was forced to close inbound, without having received the proper SSL/TLS close notification message from the peer, due to end of stream.");
        }
        this.closeConnection();
    }
    ...
    static {
        log = LoggerFactory.getLogger(SSLSocketChannel.class);
    }
}
