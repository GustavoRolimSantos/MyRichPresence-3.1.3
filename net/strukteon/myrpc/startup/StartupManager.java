...
public class StartupManager
{
    ...
    public static void main(final String[] args) {
        if (!Settings.initFolder()) {
            JOptionPane.showMessageDialog(StartupManager.frame, "The Program has no writing permissions in this directory, please move it to another directory or start it with admin privileges.");
            return;
        }
        ...
        layout.setVerticalGroup(layout.createSequentialGroup().addComponent(StartupManager.label).addComponent(StartupManager.progressBar).addComponent(StartupManager.cancelButton));
        layout.setHorizontalGroup(layout.createParallelGroup(GroupLayout.Alignment.CENTER).addComponent(StartupManager.label).addComponent(StartupManager.progressBar).addComponent(StartupManager.cancelButton));
        ...
        if (SystemUtils.IS_OS_WINDOWS) {
            downloadUrl = String.format("https://gluonhq.com/download/javafx-%s-sdk-windows/", "11.0.1".replace('.', '-'));
        }
            ...
            downloadUrl = String.format("https://gluonhq.com/download/javafx-%s-sdk-linux/", "11.0.1".replace('.', '-'));
        }
    ...
    private static void restart(final String modulePath) throws IOException, InterruptedException {
        ...
        final String[] args = { path, "--module-path", (SystemUtils.IS_OS_WINDOWS ? "\"" : "") + modulePath.replace('\\', '/') + "/lib" + (SystemUtils.IS_OS_WINDOWS ? "\"" : ""), "--add-modules", "javafx.controls,javafx.fxml,javafx.web", "-jar", programPath, "javafx-installed" };
