...
import net.strukteon.myrpc.websocket.*;
import java.awt.*;
import java.net.*;
...
public class MyRichPresence {
	...
	public List<String> getProfileNames() {
		return this.profiles.stream().map(p -> p.profileName)
	...
	public void updatePresence() {
		...
		if (!this.running) {
			this.lib.Discord_Initialize(p.appId, null, false, "");
		}
		this.presence = new DiscordRichPresence();
	...
	public void handleExtensionPresence(final SocketMessage socketMessage) {
		if (!this.running) {
			this.lib.Discord_Initialize(socketMessage.service.getAppId(), null, false, "");
		}
		this.presence = new DiscordRichPresence();
