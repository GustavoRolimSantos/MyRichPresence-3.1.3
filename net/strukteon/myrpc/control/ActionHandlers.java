...
public class ActionHandlers
{
    ...
    public void openPreview(final ActionEvent e) {
        MyRichPresence.openWebpage(String.format("https://discordapp.com/developers/applications/%s/rich-presence/visualizer", this.gui.myRichPresence.getCurrentProfile().appId));
    }
    ...
    public void onAddProfileClick(final ActionEvent e) {
        ...
        final Optional<String> answer = tid.showAndWait();
        if (answer.isPresent() /*&& !this.gui.myRichPresence.getCurrentProfile().profileName.stream().map(p -> p.profileName.toLowerCase()).collect((Collector<? super Object, ?, List<? super Object>>)Collectors.toList()).contains(answer.get().toLowerCase())*/) {
            try {
    ...
    public void onCurrentPlayersChange(final ObservableValue<? extends String> observable, final String oldValue, final String newValue) {
    ...
    public void onMaximumPlayersChange(final ObservableValue<? extends String> observable, final String oldValue, final String newValue) {
        if (newValue.matches("0[0-9]")) {
    ...
    public void onDownloadExtensionClick(final ActionEvent e) {
        ...
        if (opt.isPresent()) {
            if (cd.getItems().get(0).equals(opt.get())) {
                ...
                MyRichPresence.openWebpage("https://chrome.google.com/webstore/detail/my-rich-presence/cgeaniepnccnmoogokikkplihmgmhocl");
            }
        }
    }
}
